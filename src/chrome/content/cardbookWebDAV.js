...
if ("undefined" == typeof(cardbookWebDAV)) {
	...
	function cardbookWebDAV(connection, target, etag, asJSON) {
		...
		this.target = target;
		this.etag = etag;
		var requestsTimeout = cardbookRepository.cardbookPreferences.getStringPref("extensions.cardbook.requestsTimeout");
		this.addonVersion = cardbookRepository.cardbookPreferences.getStringPref("extensions.cardbook.addonVersion");
	...
	cardbookWebDAV.prototype = {
		...
		setDigestCredentials: function (aHeader, aXhrOrig, aMethod, aBody) {
			...
			var shortUrl = this.url.replace(cardbookRepository.cardbookSynchronization.getRootUrl(this.url), '');
		...
		setCredentials: function (aHeader) {
			if (this.accessToken) {
				...
				if (!this.username) {
					if (this.prefId) {
						this.username = cardbookRepository.cardbookPreferences.getUser(this.prefId);
					} else {
						this.username = "";
					}
				}
		...
		dumpSecurityInfo: function (aChannel) {
			...
			try {
				...
				if (secInfo instanceof Components.interfaces.nsITransportSecurityInfo) {
					...
					cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Security state of connection :");
					...
					if ((secInfo.securityState & Components.interfaces.nsIWebProgressListener.STATE_IS_SECURE) == Components.interfaces.nsIWebProgressListener.STATE_IS_SECURE) {
						cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Secure connection");
						...
						cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Insecure connection");
						...
						cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Unknown");
						cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Security description : " + secInfo.shortSecurityDescription);
						cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : Security error message : " + secInfo.errorMessage);
					}
				} else {
					cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(this.logDescription + " : debug mode : No security info available for this channel");
				}
			}
			catch(e) {
		...
		makeHTTPRequest: function(method, body, headers, aCleanBody, aXhrOrig) {
			...
			if (aCleanBody) {
				cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : body : ", aCleanBody);
			} else {
				cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : body : ", body);
			}
			cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : username : ", this.username);
			cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : url : ", this.url);
		...
		sendHTTPRequest: function(method, body, headers, aCleanBody, aXhrOrig, aOverrideMime) {
			try {
				...
				// Services.prompt.alert(null, errorTitle, e);
				cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : 666 error : ", e);
		...
		handleHTTPResponse: function(aChannel, aStatus, aResultLength, aResult) {
			...
			if (!this.hideResponse) {
				cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : response text : ", aResult);
		...
		_formatRelativeHref: function(aString) {
			...
			try {
				decodeReport = cardbookRepository.cardbookPreferences.getBoolPref("extensions.cardbook.decodeReport");
			} catch (e) {
				decodeReport = true;
			}
			...
			cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug1(this.logDescription + " : debug mode : can not parse relative href : ", aString);
			return "";
		}
	};
};
