...
(function() {
	// Keep a reference to the original function.
	ovl_cardbookComposeMsg.origFunctions.focusContactsSidebarSearchInput = focusContactsSidebarSearchInput;

	// Override a function.
	focusContactsSidebarSearchInput = function () {
		if (document.getElementById("contactsSplitter").isCollapsed) {
			return false;
		}
		let input = document.getElementById("contactsBrowser").contentDocument.getElementById("cardbookpeopleSearchInput");
		if (!input) {
			return false;
		}
		input.focus();
...
setContactsSidebarVisibility(!document.getElementById("contactsSplitter").isCollapsed, false);
