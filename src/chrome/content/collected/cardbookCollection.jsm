			...
			for (let address of addresses) {
				...
				if (address.name) {
				 ...
				 if (!cardbookRepository.isEmailRegistered(address.email)) {
						...
						if (!cardbookRepository.cardbookPreferences.getReadOnly(dirPrefId)) {
							if (resultEmailsCollections[i][0] == "true") {
								if ((aIdentity == resultEmailsCollections[i][1]) || ("allMailAccounts" == resultEmailsCollections[i][1])) {
									...
									cardbookRepository.cardbookLog.updateStatusProgressInformationWithDebug2(dirPrefIdName + " : debug mode : trying to collect contact " + address.name + " (" + address.email + ")");
									await cardbookRepository.cardbookUtils.addCardFromDisplayAndEmail(dirPrefId, address.name, address.email, resultEmailsCollections[i][3], myActionId);
								}
							}
						}
					}
					...
					if (cardbookRepository.isEmailRegistered(address.email) && address.name) {
