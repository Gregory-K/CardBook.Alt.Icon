export class TreeSelection {
		...
		if (this._count == 0) {
			...
			for (let [iTupe, [low, high]] of this._ranges.entries()) {
				...
				if (index == high + 1) {
					...
					//  the two ranges.
					if (iTupe + 1 < this._ranges.length && this._ranges[iTupe + 1][0] == index + 1) {
	...
	*/
	rangedSelect(rangeStart, rangeEnd, augment) {
		...
		for (let [iTupe, [low, high]] of this._ranges.entries()) {
			...
			if (rangeStart >= low - 1 && rangeStart <= high + 1 && lowOverlap == null) {
		...
		if (lowNuke != null) {
			this._ranges.splice(lowNuke, highNuke - lowNuke + 1, [ rangeStart, rangeEnd ]);
		} else {
	...
	*/
	...
	adjustSelection(index, count) {
		...
		// adjust our points
		this._shiftSelectPivot = this._adjustPoint(this._shiftSelectPivot, index, count);
		...
		if (iTrans > 0 && iTrans < this._ranges.length && this._ranges[iTrans - 1][1] == this._ranges[iTrans][0]) {
