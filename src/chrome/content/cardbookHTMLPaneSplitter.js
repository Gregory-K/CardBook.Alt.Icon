class CBPaneSplitter extends HTMLHRElement {
	...
	_updateResizeElement(element) {
		...
		this._beforeElement = element && !!(this.compareDocumentPosition(element) & Node.DOCUMENT_POSITION_FOLLOWING);
		// Are we already collapsed?
		this._isCollapsed = this._resizeElement?.classList.contains("collapsed-by-splitter");
		this._updateStyling();
	}
	...
	*/
	collapse() {
		...
		this.dispatchEvent(new CustomEvent("splitter-collapsed", { bubbles: true }));
	}
	...
	*/
	expand() {
		...
		this._updateStyling();
		this._updateDragCursor();
		this.dispatchEvent(new CustomEvent("splitter-expanded", { bubbles: true }));
	}
	...
	*/
	_updateStyling() {
		...
		if (this.id != this._cssName?.basis) {
			...
			this._cssName = {basis: this.id, height: `--${this.id}-height`, width: `--${this.id}-width`};
		}
		...
		this.resizeElement.classList.toggle("collapsed-by-splitter", this.isCollapsed);
	...
	_onMouseDown(event) {
		...
		let vertical = this.resizeDirection == "vertical";
		let collapseSize = Number(this.getAttribute(vertical ? "collapse-height" : "collapse-width")) || 0;
		...
		this._dragStartInfo = {
			...
			negative: vertical ? this._beforeElement : this._beforeElement == ltrDir,
	...
	*/
	...
	_onMouseMove(event) {
		...
		let { wasCollapsed, vertical, negative, pos, size, collapseSize } = this._dragStartInfo;
		...
		if (!this._started) {
			...
			this._started = true;
			this.dispatchEvent(new CustomEvent("splitter-resizing", { bubbles: true }));
		}
	...
	*/
	endResize() {
		...
		if (didStart) {
			this.dispatchEvent(new CustomEvent("splitter-resized", { bubbles: true }));
		}
	}
}
customElements.define("cb-pane-splitter", CBPaneSplitter, { extends: "hr" });
